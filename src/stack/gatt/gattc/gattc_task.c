...
#if (DBG_DBG_GATTC_TASK_EN)
#define DBG_GATTC_TASK(x)           printf x
#define DBG_GATTC_TASK_FUNC()       printf("%s\n", __func__)
#else // !(DBG_DBG_GATTC_TASK_EN)
#define DBG_GATTC_TASK(x)
#define DBG_GATTC_TASK_FUNC()
#endif // (DBG_DBG_GATTC_TASK_EN)
/*
...
__STATIC int gattc_process_op(uint8_t conidx, uint8_t op_type, void* op_msg, enum gattc_operation* supp_ops)
{
    ...
    uint8_t  state = ((ke_state_get_handler)SVC_ke_state_get)(KE_BUILD_ID(TASK_GATTC, conidx));
    ...
    {
        gattc_send_error_evt(conidx, operation, seq_num, ((ke_msg_src_id_get_handler)SVC_ke_msg_src_id_get)(op_msg), status);
    }
                                     ...
                                     ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if(msg_status == KE_MSG_NO_FREE)
    {
        ...
        req->mtu_size = ((gapm_get_max_mtu_handler)SVC_gapm_get_max_mtu)();
    ...
    if(msg_status == KE_MSG_NO_FREE)
    {
        ...
        switch(param->operation)
        {
            ...
            case GATTC_WRITE_SIGNED:
            {
                ...
                if (((gapc_is_sec_set_handler)SVC_gapc_is_sec_set)(conidx, GAPC_LK_BONDED) && (((gapc_lk_sec_lvl_get_handler)SVC_gapc_lk_sec_lvl_get)(conidx) > GAP_LK_NO_AUTH))
                {
        ...
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if(((ke_state_get_handler)SVC_ke_state_get)(dest_id) != GATTC_FREE)
    {
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if(msg_status == KE_MSG_NO_FREE)
    {
        ...
        if(param->operation == GATTC_UNREGISTER)
        {
            ...
            {
                //unregister event
                ((co_list_extract_handler)SVC_co_list_extract)(&(gattc_env[conidx]->client.reg_evt), &(reg_evt->hdr));
                ((ke_free_handler)SVC_ke_free)(reg_evt);
            }
            else
            {
        ...
        else
        {
            ...
            else
            {
                ...
                reg_evt = (struct attc_register_evt*)((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct attc_register_evt), KE_MEM_ATT_DB);
                // register event
                ...
                ((co_list_push_back_handler)SVC_co_list_push_back)(&(gattc_env[conidx]->client.reg_evt), &(reg_evt->hdr));
            }
        }
        /* send discovery complete event with error */
        gattc_send_complete_evt(conidx, GATTC_OP_CLIENT, status);
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if(msg_status == KE_MSG_NO_FREE)
    {
        ...
        else
        {
            ...
            ((ke_msg_send_handler)SVC_ke_msg_send)(svc_disc);
        }
    }
        ...
        else
        {
            ...
            switch(evt->operation)
            {
                case GATTC_DISC_ALL_CHAR:
                {
					...
					uint16_t info_idx;
					for(info_idx = 0; info_idx <(svc->end_hdl - svc->start_hdl); info_idx++)
					{	
						if(svc->info[info_idx].att_type == GATTC_SDP_NONE)
							break;	
					}
					if(info_idx < (svc->end_hdl - svc->start_hdl))
					{
						disc_type = GATTC_DISC_DESC_CHAR;
						break;
					}
                }
				// no break, after read by type, no need information find , continue next svc grp
                case GATTC_DISC_DESC_CHAR:
                {
                    ...
                    if(svc != NULL)
                    {
                        ...
                        ((co_list_pop_front_handler)SVC_co_list_pop_front)(&(gattc_env[conidx]->client.sdp_data));
                        // inform that message has been received
                        ((ke_msg_send_handler)SVC_ke_msg_send)(svc);
                        svc = NULL;
                    }
                    else
                    {
            ...
            if(svc_search)
            {
                ...
                while(! co_list_is_empty(&(gattc_env[conidx]->client.sdp_data)))
                {
                    ...
                    msg = (struct ke_msg*) ((co_list_pop_front_handler)SVC_co_list_pop_front)(&(gattc_env[conidx]->client.sdp_data));
                    ...
                    else
                    {
						...
						if((svc_ind->end_hdl - svc_ind->start_hdl) >= 20)
						{
							svc_ind->end_hdl = svc_ind->start_hdl + 20;
						}
                        ...
                        if(!((ke_check_malloc_handler)SVC_ke_check_malloc)(svc_msg_size, KE_MEM_KE_MSG))
                        {
                        ...
                        else
                        {
                            ...
                            ((co_list_push_front_handler)SVC_co_list_push_front)(&(gattc_env[conidx]->client.sdp_data),
            ...
            if(disc_type != GATTC_NO_OP)
            {
                if(svc != NULL)
                {
					...
					if((svc->end_hdl - svc->start_hdl) >= 20)
					{
						svc->end_hdl = svc->start_hdl + 20;
					}
                    disc->operation = disc_type;
                    disc->start_hdl = svc->start_hdl;	//svc->start_hdl+1;    //DYC changed
                    disc->end_hdl   = svc->end_hdl;
//DYC added, only for information found
					if(disc_type == GATTC_DISC_DESC_CHAR)
					{
						uint16_t idx;
						for(idx = svc->start_hdl+1; idx<=svc->end_hdl; idx++)
						{	if(svc->info[idx-(svc->start_hdl+1)].att_type == GATTC_SDP_NONE)
							{	
								disc->start_hdl = idx;
								disc->end_hdl = idx;
								break;
						...
						/*
						for(idx = disc->start_hdl; idx<=svc->end_hdl; idx++)
						{	if(svc->info[idx-(svc->start_hdl+1)].att_type == GATTC_SDP_NONE)
							{	
								disc->end_hdl = idx;
							}
                    ...
                    ((ke_msg_send_handler)SVC_ke_msg_send)(disc);
                }
                else
                {
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if((opcode == GATTC_SDP_DISC_SVC) || (opcode == GATTC_SDP_DISC_SVC_ALL))
    {
        ...
        ((co_list_push_back_handler)SVC_co_list_push_back)(&(gattc_env[conidx]->client.sdp_data), &(ke_param2msg(ind)->hdr));
    }
                                              ...
                                              ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if (((ke_state_get_handler)SVC_ke_state_get)(dest_id) != GATTC_FREE)
    {
            ...
            (param->svc_shdl != 0x0000))
        {
            ...
            if((!((gapm_svc_chg_en_handler)SVC_gapm_svc_chg_en)()) || (!((gapc_svc_chg_ccc_get_handler)SVC_gapc_svc_chg_ccc_get)(conidx)))
            {
				printf("gattc_send_svc_changed_cmd_handler\n");
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if (((ke_state_get_handler)SVC_ke_state_get)(dest_id) != GATTC_FREE)
    {
        ...
        {
            switch(gattc_env[conidx]->server.pdu->pdu.data.code)
            {
                ...
                case L2C_CODE_ATT_EXE_WR_REQ:
                {
                    ...
                    else
                    {
                        ...
                        ((ke_msg_forward_handler)SVC_ke_msg_forward)(gattc_env[conidx]->server.pdu, dest_id, dest_id);
                    }
                }
                break;
                ...
                default:
                {
                    ASSERT_INFO(0 , conidx, gattc_env[conidx]->server.pdu->pdu.data.code);
                    /* Ignore message */
                }
                break;
            }
            ...
            switch(gattc_env[conidx]->server.pdu->pdu.data.code)
            {
                case L2C_CODE_ATT_PREP_WR_REQ:
                {
                    ...
                    if(param->status == ATT_ERR_NO_ERROR)
                    {
                        ...
                        gattc_env[conidx]->server.pdu->pdu.payld_len = param->length | PERM(RI, ENABLE);
                        // continue PDU procedure execution
                        ((ke_msg_forward_handler)SVC_ke_msg_forward)(gattc_env[conidx]->server.pdu, dest_id, dest_id);
                    }
                    else
                   ...
                   ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if(state != GATTC_FREE)
    {
        ...
        if((!gattc_env[conidx]->trans_timeout) && (!((gapc_is_disc_connection_handler)SVC_gapc_is_disc_connection)(conidx)))
        {
            ((ke_msg_send_basic_handler)SVC_ke_msg_send_basic)(GATTC_TRANSACTION_TO_ERROR_IND, TASK_APP, dest_id);
        }
                             ...
                             ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if(state != GATTC_FREE)
    {
        ...
        else if(param->status == L2C_ERR_INVALID_PDU)
        {
            ...
            ((l2cc_pdu_send_handler)SVC_l2cc_pdu_send)(err_msg);
        }
    }
                                       ...
                                       ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if(state != GATTC_FREE)
    {
                ...
                (ccc_value == ATT_CCC_START_IND))
        {
            ...
            // set value
            ((gapc_svc_chg_ccc_set_handler)SVC_gapc_svc_chg_ccc_set)(conidx, (ccc_value == ATT_CCC_START_IND));
                                       ...
                                       ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if (param->handle == GATT_GET_ATT_HANDLE(GATT_IDX_SVC_CHANGED_CFG))
    {
        ...
        co_write16p(cfm->value, (((gapc_svc_chg_ccc_get_handler)SVC_gapc_svc_chg_ccc_get)(conidx) ? ATT_CCC_START_IND : ATT_CCC_STOP_NTFIND));
        status = ATT_ERR_NO_ERROR;
    }
...
const struct ke_state_handler task_gattc_msg_default_handler = KE_STATE_HANDLER(gattc_msg_handler_tab);
/// GATT task instance.
ke_state_t gattc_state[GATTC_IDX_MAX];


/// GATTC task descriptor
const struct ke_task_desc TASK_DESC_GATTC = {NULL,&task_gattc_msg_default_handler, gattc_state, GATTC_IDX_MAX, 1,ARRAY_LEN(gattc_msg_handler_tab)};
