                                     ...
                                     struct attm_elmt * elmt)
{
    ...
    do
    {
        ...
        if(sec_lvl >= PERM_RIGHT_UNAUTH)
        {
            ...
            if(((gapc_lk_sec_lvl_get_handler)SVC_gapc_lk_sec_lvl_get)(conidx) == GAP_LK_NO_AUTH)
            {
            ...
            if (access != ATT_WRITE_SIGNED_ACCESS)
            {
                ...
                if(!((gapc_is_sec_set_handler)SVC_gapc_is_sec_set)(conidx, GAPC_LK_ENCRYPTED))
                {
                    // check if LTK exchanged during pairing
                    if(((gapc_is_sec_set_handler)SVC_gapc_is_sec_set)(conidx, GAPC_LK_LTK_PRESENT))
                    {
                ...
                if (eks_req && (((gapc_enc_keysize_get_handler)SVC_gapc_enc_keysize_get)(conidx) < ATT_SEC_ENC_KEY_SIZE))
                {
...
//printf("ATTM_GET_VALUE,HDL: %d\n",handle);
    ...
    if(status == ATT_ERR_REQUEST_NOT_SUPPORTED)
    {
        ...
        ((ke_msg_send_handler)SVC_ke_msg_send)(rd_req_ind);
    }
                         ...
                         uint16_t offset, uint16_t handle)
{
    ...
    // And send the message
    ((ke_msg_send_handler)SVC_ke_msg_send)(wr_ind);
}
    ...
    if(status == ATT_ERR_NO_ERROR)
    {
        ...
        while (elem != NULL)
        {
            ...
            if(handle < ATTS_PREP_WRITE_GET(elem).handle)
            {
                ...
                ((co_list_insert_before_handler)SVC_co_list_insert_before)((&gattc_env[conidx]->server.prep_wr_req_list),
        ...
        if (elem == NULL)
        {
            ...
            ((co_list_push_back_handler)SVC_co_list_push_back)((&gattc_env[conidx]->server.prep_wr_req_list),
...
static void atts_send_pdu(void *pdu)
{
    ...
    ((l2cc_pdu_send_handler)SVC_l2cc_pdu_send)(pdu);
}
...
uint8_t atts_send_event(uint8_t conidx, struct gattc_send_evt_cmd *event)
{
    ...
    if(status == ATT_ERR_NO_ERROR)
    {
        ...
        {
            /* start the timer */
            ((ke_timer_set_handler)SVC_ke_timer_set)(GATTC_SERVER_RTX_IND, KE_BUILD_ID(TASK_GATTC, conidx), ATT_TRANS_RTX);
        }
    }
...
void atts_clear_prep_data(uint8_t conidx)
{
    ...
    while(!co_list_is_empty(&(gattc_env[conidx]->server.prep_wr_req_list)))
    {
        struct atts_prep_data *prep = (struct atts_prep_data*)
                            ((co_list_pop_front_handler)SVC_co_list_pop_front)(&(gattc_env[conidx]->server.prep_wr_req_list));
        ((ke_free_handler)SVC_ke_free)(prep);
    }
...
void atts_clear_rsp_data(uint8_t conidx)
{
    ...
    {
        struct co_list_hdr * data = ((co_list_pop_front_handler)SVC_co_list_pop_front)(&(gattc_env[conidx]->server.rsp));
        ((ke_free_handler)SVC_ke_free)(data);
    }
    ((co_list_init_handler)SVC_co_list_init)(&gattc_env[conidx]->server.rsp);

    // clean-up received PDU
    if((gattc_env[conidx]->server.pdu != NULL)
            && !(((ke_msg_in_queue_handler)SVC_ke_msg_in_queue)(gattc_env[conidx]->server.pdu)))
    {
        ((ke_msg_free_handler)SVC_ke_msg_free)(ke_param2msg(gattc_env[conidx]->server.pdu));
        gattc_env[conidx]->server.pdu = NULL;
    }

    // Put ATTS into a ready state
    gattc_update_state(conidx, GATTC_ATTS_BUSY, false);
}
...
void atts_clear_read_cache(uint8_t conidx)
{
    if(gattc_env[conidx]->server.read_cache != NULL)
    {
        ((ke_msg_free_handler)SVC_ke_msg_free)(ke_param2msg(gattc_env[conidx]->server.read_cache));
...
{
    // Maximal MTU
    uint16_t max_mtu = ((gapm_get_max_mtu_handler)SVC_gapm_get_max_mtu)();

    struct l2cc_att_mtu_rsp* mtu_rsp = ATTS_ALLOCATE_PDU(conidx,
            L2C_CODE_ATT_MTU_RSP, l2cc_att_mtu_rsp, 0);

    // Set the MTU value
    gattc_set_mtu(conidx, co_min(((gapm_get_max_mtu_handler)SVC_gapm_get_max_mtu)(), req->mtu_size));
        ...
        do
        {
            ...
            if(handle <= req->ehdl)
            {
                struct atts_find_info_rsp* info = (struct atts_find_info_rsp*)
                        ((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct atts_find_info_rsp), KE_MEM_KE_MSG);
                ...
                if(length != info->uuid_len)
                {
                    ...
                    ((ke_free_handler)SVC_ke_free)(info);
                    // stop loop
                    break;
                }
                else
                {
                    ...
                    ((co_list_push_back_handler)SVC_co_list_push_back)(&(gattc_env[conidx]->server.rsp), &(info->hdr));
...
static int atts_find_by_type_req(uint8_t conidx, struct l2cc_att_find_by_type_req* req)
{
        ...
        do
        {
                    ...
                    || ((length == req->val_len) && (memcmp(req->val, value, req->val_len) == 0)))
            {
                struct atts_find_by_type_rsp* find =
                        (struct atts_find_by_type_rsp*)((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct atts_find_by_type_rsp), KE_MEM_KE_MSG);
                ...
                // put element at end of the list
                ((co_list_push_back_handler)SVC_co_list_push_back)(&(gattc_env[conidx]->server.rsp), &(find->hdr));
    ...
    if (status != ATT_ERR_REQUEST_NOT_SUPPORTED)
    {
        uint16_t size = ((co_list_size_handler)SVC_co_list_size)(&(gattc_env[conidx]->server.rsp));
        ...
        do
        {
            ...
            else
            {
                ...
                struct atts_rd_by_type_rsp* rd =
                        (struct atts_rd_by_type_rsp*)((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct atts_rd_by_type_rsp) + val_length, KE_MEM_KE_MSG);
                ...
                *remaining_len -= ATT_HANDLE_LEN + val_length;
                ((co_list_push_back_handler)SVC_co_list_push_back)(&(gattc_env[conidx]->server.rsp), &(rd->hdr));
        ...
        do
        {
            ...
            else
            {
                ...
                struct atts_rd_by_grp_type_rsp* rd =
                       (struct atts_rd_by_grp_type_rsp*) ((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct atts_rd_by_grp_type_rsp) + val_length, KE_MEM_KE_MSG);
...
static int atts_read_mult_req(uint8_t conidx, struct l2cc_att_rd_mult_req* req)
{
    ...
    // retrieve stored parameters to continue execution
    uint16_t cursor = ((co_list_size_handler)SVC_co_list_size)(&(gattc_env[conidx]->server.rsp));
    uint16_t *remaining_len = ATTS_REMAINING_LEN(req);
            ...
            && (status == ATT_ERR_NO_ERROR))
    {
        ...
        if (status == ATT_ERR_NO_ERROR)
        {
            ...
            struct atts_rd_mult_rsp* val =
                    (struct atts_rd_mult_rsp*)((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct atts_rd_mult_rsp) + value_len, KE_MEM_KE_MSG);
            ...
            ((co_list_push_back_handler)SVC_co_list_push_back)(&(gattc_env[conidx]->server.rsp), &(val->hdr));
...
static int atts_write_signed(uint8_t conidx, struct l2cc_att_sign_wr_cmd* cmd)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        // Send the message
        ((ke_msg_send_handler)SVC_ke_msg_send)(sign_cmd);
...
static int atts_prepare_write_req(uint8_t conidx, struct l2cc_att_prep_wr_req* req)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        if(!atts_hdl_present_in_prep_data(conidx, req->handle) && (*current_length == 0))
        {
            ...
            // And send the message
            ((ke_msg_send_handler)SVC_ke_msg_send)(info_req_ind);
        ...
        else
        {
            ...
            status = atts_store_prep_data(conidx, ke_param2msg(L2CC_PDU_TO_IND(req)),
                    req->value_len, req->offset, req->handle, PERM_GET(*current_length, MAX_LEN));
            ...
            if (status == ATT_ERR_NO_ERROR)
            {
                ...
                // message is reused in prepare write list, update state of GATT for ATTS here
                gattc_env[conidx]->server.pdu = NULL;
                // Put ATTS into a ready state
                gattc_update_state(conidx, GATTC_ATTS_BUSY, false);
                msg_status = KE_MSG_NO_FREE;
...
const struct att_pdu_handler atts_handlers[] =
{
    ...
    { L2C_CODE_ATT_HDL_VAL_CFM,             (att_func_t) atts_hdl_value_cfm },
};
#endif /* (BLE_ATTS) */
...
int atts_l2cc_pdu_recv_handler(uint8_t conidx, struct l2cc_pdu_recv_ind *param)
{
    ...
    // search PDU Handler
    for(cursor = 0 ; cursor < (sizeof(atts_handlers) / sizeof(struct att_pdu_handler)) ; cursor++)
    {
    ...
    if(fhandler != NULL)
    {
        ...
        if (((ke_state_get_handler)SVC_ke_state_get)(KE_BUILD_ID(TASK_GATTC, conidx)) & GATTC_CONNECTED)
        {
        ...
        else
        {
            ...
            // check if there is PDU can be handled (no pdu process on-going, or received pdu where process is on-going)
            else if((gattc_env[conidx]->server.pdu == NULL) || (gattc_env[conidx]->server.pdu == param))
            {
                ...
                if(gattc_env[conidx]->server.pdu == NULL)
                {
                    ...
                    gattc_env[conidx]->server.pdu = param;

                    // Put ATTS into a busy state
                    gattc_update_state(conidx, GATTC_ATTS_BUSY, true);
                }
                // execute PDU Handler
                msg_status = fhandler(conidx, &(param->pdu.data.code));

                // message has been consumed, perform a clean-up
                if(msg_status == KE_MSG_CONSUMED)
                {
                    // Clear Response data
                    atts_clear_rsp_data(conidx);
                    // PDU already free during clean-up;
                    msg_status = KE_MSG_NO_FREE;
                }
            }
            else
            {
                // PDU can not be handled immediately, put it in saved queue.
                msg_status = KE_MSG_SAVED;
            }
        }
    }
    ...
    switch (param->pdu.data.code)
    {
        ...
        case L2C_CODE_ATT_EXE_WR_REQ:
        {
            ...
            /* send the L2Cap message */
            ((l2cc_pdu_send_handler)SVC_l2cc_pdu_send)(err_rsp);
