...
void attc_send_att_req(uint8_t conidx, void *pdu)
{
    ...
    if (!((pkt->pdu.data.code == L2C_CODE_ATT_SIGN_WR_CMD)
            ...
            || (pkt->pdu.data.code == L2C_CODE_ATT_MTU_REQ)
            || (pkt->pdu.data.code == L2C_CODE_ATT_HDL_VAL_CFM)
            || ((pkt->pdu.data.code == L2C_CODE_ATT_EXE_WR_REQ)
                    && (pkt->pdu.data.exe_wr_req.flags == 0x00))))
    {
        /* start the timer; will destroy the link if it expires */
        ((ke_timer_set_handler)SVC_ke_timer_set)(GATTC_CLIENT_RTX_IND, KE_BUILD_ID(TASK_GATTC, conidx), ATT_TRANS_RTX);
    }
...
void attc_send_read_ind(uint8_t conidx)
{
    ...
    if(!co_list_is_empty(&(gattc_env[conidx]->client.rsp_list)))
    {
        ...
        ((ke_msg_send_handler)SVC_ke_msg_send)(read_ind);
    }
}
    ...
    switch(gattc_get_operation(conidx, GATTC_OP_CLIENT))
    {
        case GATTC_DISC_DESC_CHAR:
        {
            ...
            while (cursor < rsp->data_len)
            {
                ...
                ((ke_msg_send_handler)SVC_ke_msg_send)(char_desc);
            }

		#if 1
			//DYC changed, to search next uninserted handle.
			...
			if((msg != NULL) && (msg->id == GATTC_SDP_SVC_IND))
			{
				...
				uint16_t idx;
				for(idx = (handle+1); idx <= svc->end_hdl; idx++)
				{	
					if(svc->info[idx-(svc->start_hdl+1)].att_type == GATTC_SDP_NONE)
					{	
						disc_cmd->start_hdl = idx;
						disc_cmd->end_hdl = idx;
						break;
					}
				}
			}
            ...
            {
                // continue discovery execution , DYC changed
    ...
    switch(gattc_get_operation(conidx, GATTC_OP_CLIENT))
    {
        case GATTC_DISC_BY_UUID_SVC:
        {
            ...
            while(cursor < rsp->data_len)
            {
                ...
                ((ke_msg_send_handler)SVC_ke_msg_send)(svc_ind);
            }
 ...
 ****************************************************************************************
 */
//int gattc_disc_char_ind_handler(ke_msg_id_t const msgid, struct gattc_disc_char_ind *ind,
...
// DYC added,  fast hdl read_by_type info.  to decide if need to send info found
				#if 1
                    // send discovered characteristic ,
                    ((ke_msg_send_handler)SVC_ke_msg_send)(char_ind);
				#else				
					struct ke_msg * msg = ke_param2msg(char_ind);
					gattc_disc_char_ind_handler(msg->id,char_ind,msg->dest_id, msg->src_id);
        ...
        case GATTC_DISC_INCLUDED_SVC:
        {
            ...
            while(cursor < rsp->data_len)
            {
                ...
                else
                {
                    ...
                    ((co_list_push_back_handler)SVC_co_list_push_back)(&(gattc_env[conidx]->client.rsp_list), &(ke_param2msg(inc_svc_ind)->hdr));
                    break;
                }
                ...
                ((ke_msg_send_handler)SVC_ke_msg_send)(inc_svc_ind);
            }
        ...
        case GATTC_READ_BY_UUID:
        {
            ...
            else
            {
                ...
                ((co_list_push_back_handler)SVC_co_list_push_back)(&(gattc_env[conidx]->client.rsp_list), &(ke_param2msg(pdu)->hdr));
        ...
        case GATTC_DISC_INCLUDED_SVC:
        {
            ...
            struct gattc_disc_svc_incl_ind* svc_incl_ind =
                    (struct gattc_disc_svc_incl_ind*) ke_msg2param(
                            (struct ke_msg*) ((co_list_pop_front_handler)SVC_co_list_pop_front)(&(gattc_env[conidx]->client.rsp_list)));

            memcpy(svc_incl_ind->uuid, &rsp->value, ATT_UUID_128_LEN);
            svc_incl_ind->uuid_len = ATT_UUID_128_LEN;

            /* send the message */
            ((ke_msg_send_handler)SVC_ke_msg_send)(svc_incl_ind);
