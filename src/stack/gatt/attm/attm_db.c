...
//show_reg2(p_cursor,ATT_UUID_128_LEN);
            // set uuid offset pointer
            new_svc->svc.uuid = (uint16_t) (((uint32_t) p_cursor) - ((uint32_t)&(new_svc->svc)));
    ...
    for(i = 0 ; i < svc_desc->nb_att; i++)
    {
        ...
        else
        {
            ...
            if(PERM_GET(svc_desc->atts[i].ext_perm, RI))	//value is not in database
            {
            ...
            else
            {
                ...
                // set uuid offset pointer
                new_svc->atts[i].info.offset  = (uint16_t) (((uint32_t) p_cursor) - ((uint32_t)&(new_svc->atts[i])));
                //new_svc->atts[i].info.offset |= PERM_GET(svc_desc->atts[i].ext_perm, UUID_LEN);
                //new_svc->atts[i].info.offset |= PERM_GET(svc_desc->atts[i].ext_perm, EKS);
								new_svc->atts[i].info.offset |= ((svc_desc->atts[i].ext_perm) & (PERM_MASK_UUID_LEN));
                new_svc->atts[i].info.offset |= ((svc_desc->atts[i].ext_perm) & (PERM_MASK_EKS));				
            }
        }
    }
}
...
uint8_t attmdb_add_service(struct gattm_svc_desc* svc_desc)
{
    ...
    do
    {
        ...
        new_svc = (struct attm_svc*) ((ke_malloc_handler)SVC_ke_malloc)(svc_mem_size, KE_MEM_ATT_DB);
...
void attmdb_destroy(void)
{
    ...
    while(current_svc != NULL)
    {
        ...
        ((ke_free_handler)SVC_ke_free)(current_svc);
    ...
    else
    {
        ...
        if(PERM_GET(elmt->info.att->info.max_length, RI) == 0)
        {
            ...
            struct attm_att_value* val =
                    (struct attm_att_value*) (((uint8_t*)elmt->info.att) + elmt->info.att->info.offset);
        ...
        else
        {
            ...
            *perm = elmt->info.att->perm;
            ...
            else if(mode_mask != PERM_MASK_ALL)
            {
                ...
                else if((svc_perm > *perm) && (*perm != PERM_RIGHT_DISABLE))
                {
                    *perm = svc_perm;
                }
            }
...
void attmdb_get_svc_info(struct gattm_svc_info* svc_info)
{
    ...
    while(current_svc != NULL)
    {
        ...
        svc_info[nb_svc].task_id   = ((gapm_get_id_from_task_handler)SVC_gapm_get_id_from_task)(current_svc->svc.task_id);
