                           ...
                           const struct attm_desc *att_db, uint8_t svc_perm)
{
    ...
    svc_desc = (struct gattm_svc_desc*) ((ke_malloc_handler)SVC_ke_malloc)(
    ...
    ((ke_free_handler)SVC_ke_free)(svc_desc);
...
//show_reg2(svc_desc->atts[nb_att].uuid,ATT_UUID_16_LEN);
        ...
        else
        {
            ...
            if(PERM_GET(elmt.info.att->info.max_length, RI) == 0)
            {
                struct attm_att_value* val = (struct attm_att_value*)
                        (((uint8_t*)elmt.info.att) + elmt.info.att->info.offset);
...
uint8_t attm_get_value(uint16_t handle, att_size_t* length, uint8_t** value)
{
    ...
    if(status == ATT_ERR_NO_ERROR)
    {
        ...
        else if (attmdb_uuid16_comp(&elmt, ATT_DECL_INCLUDE))
        {
            ...
            val = (struct attm_att_value*) (((uint8_t*)elmt.info.att) + elmt.info.att->info.offset);
		...
		//printf("RI?:%d\n",PERM_GET(elmt.info.att->info.max_length, RI));
...
#if 0
//DYC added for svc disable/enable
uint8_t attm_svc_update_permission(uint16_t handle, uint8_t access_mask, uint8_t perm)
{
    uint8_t status = ATT_ERR_NO_ERROR;
    struct attm_svc * current_svc = attmdb_get_service(handle);
    ...
    {
        /* update attribute permissions */
        current_svc->svc.perm = (current_svc->svc.perm & (~access_mask)) | perm;
    }
