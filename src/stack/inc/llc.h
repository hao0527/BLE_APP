...
struct llc_env_tag
{
    ...
    #if (BLE_CHNL_ASSESS)
    /// Channel Assessment - Number of packets received on each channel.	0x25 =39
...
#else
	struct llc_enc_key_ref_cmp_evt *event = KE_MSG_ALLOC(HCI_EVENT, TASK_LLM,
						 HCI_ENC_KEY_REFRESH_CMP_EVT_CODE, llc_enc_key_ref_cmp_evt);
#endif
    event->conhdl= conhdl;
    event->status= status;
    //ke_msg_send(event);
