...
__INLINE bool llm_util_check_adv_report_list(struct bd_addr * adv_bd_addr, uint8_t adv_type)
{
    ...
    for (uint8_t idx = 0; idx < BLE_DUPLICATE_FILTER_MAX; idx++)
    {
        ...
        {
        //addr & adv_type are same, then false.    else  recode type, and true,need send up
...
__INLINE void llm_util_set_param_adv_dft(void)
{
    ...
    // sets the type of advertising, the length(only AdvA len = 6 bytes, no AdvData)
...
__INLINE void llm_util_chk_tst_mode(void)
{
    ...
    {
RWIP_DEBUGF(LLM_DEBUG,("test mode end cmpeled\n")); 
		...
		event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, TASK_LLM, HCI_LE_TEST_END_CMD_OPCODE, llm_test_end_cmd_complete);
