...
#if (RW_DM_SUPPORT)
#define LLD_EVT_PROG_LATENCY      6
#else
        #define LLD_EVT_PROG_LATENCY      1
#endif // RW_DM_SUPPORT
...
#define LLD_EVT_DEFAULT_RX_WIN_SIZE     0xf
...
/// Delay used for BLE to compensate the clock
        #define LLD_CLOCK_CORR_LAT      2   // Maximum time for clock correction is 2 slot
...
enum
{
    LLD_NO_RESTART,
    LLD_STOP_REQUESTED,
//DYC added for program  evt 6time
    LLD_ADV_RESTART_6,
    LLD_ADV_RESTART,
    LLD_SCN_RESTART,
    LLD_MST_RESTART,
...
struct lld_evt_tag
{
    ...
    uint16_t duration;
    /// Period of the event, unit 625us.
    uint16_t interval;
    ...
    bool rx_sync_err;
    /// Flag indicating the rx status on the rx packet
    uint16_t rx_status;
...
__INLINE bool lld_evt_time_cmp(uint32_t time1,
                               uint32_t time2)
{
    //printf("cmp:%d\n",((time1 - time2) & BLE_BASETIMECNT_MASK));
