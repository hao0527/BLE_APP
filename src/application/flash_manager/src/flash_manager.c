...
PAN_MESH_STATIC_ASSERT(HEADER_LEN == WORD_SIZE);
PAN_MESH_STATIC_ASSERT(IS_WORD_ALIGNED(sizeof(flash_manager_metadata_t)));
...
packet_buffer_t m_action_queue;
uint8_t         m_action_queue_buffer[ACTION_QUEUE_BUFFER_LENGTH] __attribute__((aligned(WORD_SIZE)));
...
bearer_event_flag_t   m_processing_flag;
fm_state_t            m_state;
action_state_t        m_action_state;
uint16_t              m_token; /**< Token dealt by mesh flash that marks all flash operations complete for the current action. */
queue_t               m_memory_listener_queue;

flash_manager_queue_empty_cb_t m_queue_empty_cb;
action_t * p_current;
fm_result_t result;
 ...
 inline action_t * get_entry_action(const fm_entry_t * p_entry)
{
 ...
 action_t * reserve_action_buffer(uint32_t size)
{
 ...
 inline void commit_action_buffer(action_t * p_action)
{
 ...
 const fm_entry_t * get_last_entry(const flash_manager_t * p_manager)
{
...
const void * get_defrag_threshold(const flash_manager_t * p_manager)
{
 ...
 * the threshold, so we need to do this as a signed integer. */
 int32_t get_remaining_free_space(const flash_manager_t * p_manager)
{
 ...
 bool flash_area_is_blank(const void * p_area, uint32_t size)
{
    MESH_ASSERT(IS_WORD_ALIGNED(p_area));
    MESH_ASSERT(IS_WORD_ALIGNED(size));
 ...
 inline bool metadata_is_valid(const flash_manager_metadata_t * p_metadata)
{
 ...
 bool flash_area_is_valid(const flash_manager_t * p_manager)
{
    for (uint32_t i = 0; i < p_manager->config.page_count; i++)
    {
        if (metadata_is_valid(&p_manager->config.p_area[i].metadata))
        {
            ...
            MESH_ASSERT(p_manager->config.p_area[i].metadata.page_index == i);
            MESH_ASSERT(p_manager->config.p_area[i].metadata.pages_in_area == p_manager->config.page_count);
        }
        else
        {
            return false;
 ...
 uint32_t flash_area_build(flash_manager_t * p_manager)
{
    for (uint32_t i = 0; i < p_manager->config.page_count; i++)
    {
        ...
        if (flash_area_is_blank(&p_manager->config.p_area[i].raw[WORD_SIZE], PAGE_SIZE - WORD_SIZE))
        {
            ...
            else
            {
                ...
                p_action->params.metadata._padding = 0x1234;
				p_action->params.metadata._padding ^= p_action->params.metadata.metadata_len;
				p_action->params.metadata._padding ^= p_action->params.metadata.entry_header_length;
				p_action->params.metadata._padding ^= p_action->params.metadata.entry_type_length_bits;
				p_action->params.metadata._padding ^= p_action->params.metadata.entry_len_length_bits;
				p_action->params.metadata._padding ^= p_action->params.metadata.page_index;
				p_action->params.metadata._padding ^= p_action->params.metadata.pages_in_area;
        ...
        else
        {
             ...
             * flash manager. */
            MESH_ASSERT(metadata_is_valid(&p_manager->config.p_area[i].metadata));
 ...
 uint32_t recover_seal(flash_manager_t * p_manager)
{
 ...
 */
 uint32_t invalidate_duplicate_of_last_entry(flash_manager_t * p_manager)
{
    uint32_t status = PAN_SUCCESS;
 ...
 uint32_t get_invalid_bytes(const flash_manager_page_t * p_area, uint32_t pages)
{
    ...
    while ((p_entry->header.handle != HANDLE_BLANK) && (p_entry->header.handle != HANDLE_SEAL))
    {
        ...
        MESH_ASSERT(p_entry < p_end);
    }

    return invalid_bytes;
}

 bool validate_result(const action_t * p_action)
{
 ...
 void free_packet_buffer(packet_buffer_packet_t * p_packet_buffer)
{
 ...
 void end_action(action_t * p_action, fm_result_t result, const fm_entry_t * p_entry)
{
    ...
    switch (p_action->action)
    {
        case ACTION_TYPE_REPLACE:
            if (result == FM_RESULT_SUCCESS)
            {
                ...
                MESH_ASSERT(p_manager->internal.p_seal->header.handle == HANDLE_SEAL);
            }
        ...
        case ACTION_TYPE_INVALIDATE:
            ...
            break;
        case ACTION_TYPE_BUILD_METADATA:
            MESH_ASSERT(p_manager->internal.state == FM_STATE_BUILDING);
        ...
        case ACTION_TYPE_ERASE_AREA:
            MESH_ASSERT(result == FM_RESULT_SUCCESS);
 ...
 bool defrag_required(action_t * p_next_action)
{
...
* Action execution
******************************************************************************/
 fm_result_t execute_action_replace(action_t * p_action)
{
    ...
    else
    {
        ...
        else
        {
            ...
            MESH_ERR_CHECK(flash(p_action->p_manager->internal.p_seal,
                        &PADDING_HEADER,
    ...
    if (p_old_entry != NULL)
    {
        ...
        MESH_ERR_CHECK(flash(p_old_entry,
                    &INVALID_HEADER,
 ...
 fm_result_t execute_action_invalidate(action_t * p_action)
{
    const fm_entry_t * p_old_entry = entry_get(get_first_entry(p_action->p_manager->config.p_area),
                                               get_area_end(p_action->p_manager->config.p_area),
                                               p_action->params.entry_data.entry.header.handle);
 ...
 fm_result_t execute_action_build_metadata(action_t * p_action)
{
    ...
    if (!flash_area_is_blank(&p_page->metadata, sizeof(flash_manager_metadata_t)))
    {
        ...
        for (uint32_t i = 0; i < sizeof(flash_manager_metadata_t); ++i)
        {
            ...
            MESH_ASSERT(p_page->raw[i] == ((uint8_t *) &p_action->params.metadata)[i] ||
                            p_page->raw[i] == 0xFF);
        }
    }

    MESH_ERR_CHECK(flash(p_page,
                               &p_action->params.metadata,
 ...
 fm_result_t execute_action_recover_seal(action_t * p_action)
{
    ...
    {
        MESH_ERR_CHECK(flash(get_next_entry(p_last_entry),
                    &SEAL_HEADER,
                    sizeof(SEAL_HEADER),
                    &m_token));
    }
 ...
 fm_result_t execute_action_erase_area(action_t * p_action)
{
    MESH_ERR_CHECK(erase(p_action->p_manager->config.p_area,
                               p_action->p_manager->config.page_count * PAGE_SIZE,
                               &m_token));
    return FM_RESULT_SUCCESS;
}

 fm_result_t execute_action(action_t * p_action)
{
 ...
 bool process_action_queue(void)
{
 ...
 void flash_op_ended_callback(mesh_flash_user_t user, const flash_operation_t * p_op, uint16_t token)
{
...
{
	memset(&m_action_queue,0,sizeof(packet_buffer_t));
	memset(m_action_queue_buffer,0,ACTION_QUEUE_BUFFER_LENGTH);
        ...
        const flash_manager_config_t * p_config)
{
    ...
    MESH_ASSERT(IS_PAGE_ALIGNED(p_config->p_area));
    MESH_ASSERT(p_config->page_count < FLASH_MANAGER_PAGE_COUNT_MAX);
...
{
    MESH_ASSERT(p_manager != NULL);
    MESH_ASSERT(handle_is_valid(handle));
    MESH_ASSERT(data_length <= FLASH_MANAGER_ENTRY_MAX_SIZE);
	//data_length +=4;
    ...
    uint32_t buffer_length = data_length + ACTION_BUFFER_SIZE_ENTRY_NO_DATA + 8;
    ...
    else
    {
        ...
        p_action->params.entry_data.entry.header.len_words = 1 + 2 + ALIGN_VAL(data_length, WORD_SIZE) / WORD_SIZE ;//(crc)
        p_action->p_manager = p_manager;
...
void flash_manager_entry_commit(fm_entry_t * p_entry)
{
	...
	uint32_t checksum;
	memcpy(&checksum,&p_entry->header,4);
	for(uint8_t i = 0;i < (p_entry->header.len_words - 3);i++)
	
		checksum ^= p_entry->data[i];
	//printf("flash data checksum is %08x\n",checksum);
	p_entry->data[p_entry->header.len_words - 3] = checksum;
	...
	for(uint8_t i = 0;i < (p_entry->header.len_words - 3);i++)
		...
		checksum ^= p_entry->data[i];
	//printf("flash data checksum is %08x\n",checksum);
	p_entry->data[p_entry->header.len_words - 2] = checksum;
...
{
    MESH_ASSERT(p_listener != NULL);
    MESH_ASSERT(p_listener->callback != NULL);
...
void flash_manager_on_defrag_end(flash_manager_t * p_manager)
{
    if (p_manager != NULL)
    {
        p_manager->internal.p_seal = entry_get(get_first_entry(p_manager->config.p_area),
                                      get_area_end(p_manager->config.p_area),
                                      HANDLE_SEAL);
        MESH_ASSERT(p_manager->internal.p_seal != NULL);
        p_manager->internal.state = FM_STATE_READY;
        p_manager->internal.invalid_bytes = 0;
    }
    m_state = FM_STATE_READY;
...
void flash_manager_action_queue_empty_cb_set(flash_manager_queue_empty_cb_t queue_empty_cb)
{
    ...
    {
        MESH_ASSERT(m_queue_empty_cb == NULL);
