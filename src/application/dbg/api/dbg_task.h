...
enum
{
    ...
    #if (DSM_SUPPORT)
    DBG_DATA_STORAGE_OP_CS_EVT,
    DBG_DATA_STORAGE_IND_EVT,
    #endif //DSM_SUPPORT
    DBG_MSG_ID_EVT_LAST
};
...
#if (DSM_SUPPORT)
///HCI Debug Data Storage Operation command parameters - vendor specific
struct dbg_data_storage_op_cmd
{
    /// Operation
    uint8_t operation;
    /// Parameters
...
/// Common HCI Command status event for VS commands
struct dbg_common_stat_evt
{
    ///Status
    uint8_t status;
};
...
struct dbg_data_storage_ind_evt
{
    /// Reserved (subcode)
    uint8_t reserved;
    /// Status
