...
#include "app_hid_selfie.h"         // HID Application Selfie Definitions
...
#define DBG_APP_HID_SELFIE(x)           //printf x
#define DBG_APP_HID_SELFIE_FUNC()       //printf("%s\n", __func__);
/*
 * DEFINES
 ****************************************************************************************
 */

/// Length of the HID Selfie Report
#define APP_HID_SELFIE_REPORT_LEN       (1)
/// Length of the Report Descriptor for an HID Selfie
#define APP_HID_SELFIE_REPORT_MAP_LEN   (sizeof(app_hid_selfie_report_map))
...
/// HID Application Selfie Environment Structure
static struct app_hid_env_tag app_hid_env;


const uint8 app_hid_selfie_report_map[] =
{
...
#define CONSUMER_REPORT_KEY_REALEASED   (0)

#define CONSUMER_REPORT_KEY_VOL_UP      (1 << 0)
#define CONSUMER_REPORT_KEY_VOL_DOWN    (1 << 1)
#define CONSUMER_REPORT_KEY_POWER       (1 << 2)
#define CONSUMER_REPORT_KEY_MUTE        (1 << 3)
...
void app_hid_enable_prf(uint8_t conidx)
{
	...
	{
        ntf_cfg = *(uint16_t *)hid_selfie_get_store_info(HIDS_SELFIE_STORE_TYPE_NTF);
...
void app_hid_send_value_report(uint8_t rpt_idx, uint8_t *buf, uint8_t len)
{
    ...
    switch (app_hid_env.state)
    {
        case (APP_HID_READY):
        {
            DBG_APP_HID_SELFIE(("APP_HID_READY, nb_report %d\n", app_hid_env.nb_report));
        ...
        {
            DBG_APP_HID_SELFIE(("APP_HID_WAIT_REP\n"));
        ...
        case (APP_HID_IDLE):
        {
            DBG_APP_HID_SELFIE(("APP_HID_IDLE\n"));
        ...
        default:
        {
            DBG_APP_HID_SELFIE(("state %d (unknown)\n", app_hid_env.state));
...
void app_hid_selfie_send_key(key_value_t key, key_state_t state)
{
    DBG_APP_HID_SELFIE_FUNC();
    ...
    switch (app_hid_env.state)
    {
        case (APP_HID_READY):
        {
            ...
            if (app_hid_env.nb_report)
            {
                ...
                if (HID_SELFIE_KEY_RELEASED == state)
                {
                    req->report.value[0] = CONSUMER_REPORT_KEY_REALEASED;
                }
                else
                {
                    if (HID_SELFIE_KEY_VOL_UP == key)
                    {
                        req->report.value[0] = CONSUMER_REPORT_KEY_VOL_UP;
                    }
                    else if (HID_SELFIE_KEY_VOL_DOWN == key)
                    {
                        req->report.value[0] = CONSUMER_REPORT_KEY_VOL_DOWN;
                    }
                    else if (HID_SELFIE_KEY_POWER == key)
                    {
                        req->report.value[0] = CONSUMER_REPORT_KEY_VOL_UP | CONSUMER_REPORT_KEY_VOL_DOWN;
                    }
                    else if (HID_SELFIE_KEY_MUTE == key)
                    {
                        req->report.value[0] = CONSUMER_REPORT_KEY_MUTE;
                    }
                }
        ...
        default:
        {
            DBG_APP_HID_SELFIE(("APP_HID_STATE Unknow(%d)\n", app_hid_env.state));
            // Drop the message
        } break;
    }
}
                                      ...
                                      ke_task_id_t const              src_id )
{
    if (app_hid_env.conidx == param->conidx)
    {
        ...
        hid_selfie_store_info((void *)&param->ntf_cfg[param->conidx], HIDS_SELFIE_STORE_TYPE_NTF);
    }
                                         ...
                                         ke_task_id_t const                 src_id )
{
    ...
    if ((param->operation == HOGPD_OP_REPORT_READ) && (param->report.type == HOGPD_REPORT_MAP))
    {
        ...
        /// Report Length (uint8_t)
        req->report.length = APP_HID_SELFIE_REPORT_MAP_LEN;
        /// Report Instance - 0 for boot reports and report map
        req->report.idx = 0;
        /// Report data
        memcpy(&req->report.value[0], &app_hid_selfie_report_map[0], APP_HID_SELFIE_REPORT_MAP_LEN);
                                     ...
                                     ke_task_id_t const src_id )
{
    if (app_hid_env.conidx == param->conidx)
    {
        ...
        {
            DBG_APP_HID_SELFIE(("error conidx\n"));
