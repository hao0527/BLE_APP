...
#if (APP_HID_KEYBOARD)
/// HID Keyboadr Report Descriptor
static const uint8_t app_hid_keyboard_report_map[] =
{
	...
	0x29, 0xff, 0x81, 0x00, 0xc0, 0x05, 0x0c, 0x09, 0x01, 0xa1, 
		0x01, 0x85, 0x02, 0x19, 0x00, 0x2a, 0x9c, 0x02, 0x15, 0x00, 
...
//Restore the cfg and nb_report.
		#if (USER_SKYWORTH)
		ntf_cfg = *(uint16_t *)skyworth_get_store_info(SKYWORTH_STORE_TYPE_NTF);
		#endif
		#if (USER_HBTV)
		ntf_cfg = *(uint16_t *)hbtv_get_store_info(HBTV_STORE_TYPE_NTF);
		#endif
...
#endif
#if (USER_SKYWORTH)
		skyworth_store_info((void *)&param->ntf_cfg[param->conidx],SKYWORTH_STORE_TYPE_NTF);
#endif
#if (USER_HBTV)
		hbtv_store_info((void *)&param->ntf_cfg[param->conidx],HBTV_STORE_TYPE_NTF);
#endif
