...
void prf_register_atthdl2gatt(prf_env_t *prf_env, uint8_t conidx, struct prf_svc *svc)
{
    if(svc->shdl != ATT_INVALID_HANDLE)
    {
        ...
        ((ke_msg_send_handler)SVC_ke_msg_send)(reg);
    }
}
...
void prf_disc_svc_send(prf_env_t *prf_env, uint8_t conidx, uint16_t uuid)
{
    ...
    ((ke_msg_send_handler)SVC_ke_msg_send)(svc_req);
}
...
//DYC added   for general svc discover
                    ...
                    uint16_t handle, uint8_t* value, uint16_t length, uint8_t operation)
{
    if(handle != ATT_INVALID_HANDLE)
    {
        ...
        // Send the message
        ((ke_msg_send_handler)SVC_ke_msg_send)(wr_char);
    }
}
...
uint8_t prf_check_svc_char_validity_uuid128(uint8_t nb_chars,
                                    const struct prf_char_inf* chars,
                                    const struct prf_char_def_uuid128* chars_req)
{
	struct prf_char_def param;
	param.req_flag = chars_req->req_flag;
	param.prop_mand = chars_req->prop_mand;
	return prf_check_svc_char_validity(nb_chars,chars,&param);
}
...
uint8_t prf_check_svc_char_desc_validity_uuid128(uint8_t descs_size,
                                         const struct prf_char_desc_inf* descs,
                                         const struct prf_char_desc_def_uuid128* descs_req,
                                         const struct prf_char_inf* chars)
{
	struct prf_char_desc_def param;
	param.req_flag = descs_req->req_flag;
	param.char_code = descs_req->char_code;
	return prf_check_svc_char_desc_validity(descs_size,descs,&param,chars);
}
...
void prf_extract_svc_info_uuid128(struct gattc_sdp_svc_ind const * param,
        uint8_t nb_chars, struct prf_char_def_uuid128* chars_req, struct prf_char_inf* chars,
        uint8_t nb_descs, struct prf_char_desc_def* descs_req, struct prf_char_desc_inf* descs
                ...
                for (svc_char=0; svc_char<nb_chars ; svc_char++)
                {
                    ...
                    if((chars[svc_char].char_hdl == ATT_INVALID_HDL)
                            && attm_uuid_comp((uint8_t*)param->info[val_idx].att.uuid, param->info[val_idx].att.uuid_len
                                                , temp_for_const_uuid, ATT_UUID_128_LEN))
                    {
                        //Save properties and handles
                        chars[svc_char].char_hdl       = char_hdl;
                        chars[svc_char].val_hdl        = val_hdl;
