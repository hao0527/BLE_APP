                                   ...
                                   ke_task_id_t const src_id)
{
    ...
    if(state == BASS_IDLE)
    {
        ...
        else
        {
            ...
            ((ke_state_set_handler)SVC_ke_state_set)(dest_id, BASS_BUSY);
...
const struct ke_state_handler task_bass_msg_default_handler = KE_STATE_HANDLER(bass_msg_handler_tab);
/// GATT task instance.
//ke_state_t bass_state[BASS_IDX_MAX];
/// GAP Manager task descriptor
//const struct ke_task_desc TASK_DESC_BASS = {NULL,&task_bass_msg_default_handler, bass_state, BASS_IDX_MAX,1, ARRAY_LEN(bass_msg_handler_tab)};
...
void bass_task_init(struct ke_task_desc *task_desc)
{
	...
	task_desc->default_handler = &task_bass_msg_default_handler;
