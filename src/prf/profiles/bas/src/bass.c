...
static uint8_t bass_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct bass_db_cfg* params)
{
    ...
    if ((params->bas_nb > 0) && (params->bas_nb <= BASS_NB_BAS_INSTANCES_MAX))
    {
        ...
        bass_env = (struct bass_env_tag* ) ((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct bass_env_tag), KE_MEM_ATT_DB);
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        *start_hdl = shdl[0];
        ...
        ((ke_state_set_handler)SVC_ke_state_set)(env->task, BASS_IDLE);
    }
    else if(bass_env != NULL)
    {
        ((ke_free_handler)SVC_ke_free)(bass_env);
    }
...
static void bass_destroy(struct prf_task_env* env)
{
    ...
    {
        ((ke_free_handler)SVC_ke_free)(bass_env->operation);
    }
...
static void bass_notify_batt_lvl(struct bass_env_tag* bass_env, uint8_t conidx, uint8_t svc_idx)
{
    ...
    ((ke_msg_send_handler)SVC_ke_msg_send)(batt_lvl);
}
...
void bass_exe_operation(void)
{
    ...
    if(finished)
    {
        ...
        ((ke_state_set_handler)SVC_ke_state_set)(prf_src_task_get(&(bass_env->prf_env), 0), BASS_IDLE);
    }
}
