        ...
        if (handle != ATT_INVALID_HDL)
        {
            ...
            if (status == GAP_ERR_NO_ERROR)
            {
                ...
                while(val != NULL)
                {
                    ...
                    {
                        ((co_list_extract_handler)SVC_co_list_extract)(&(diss_env->values), &(val->hdr));
                        ((ke_free_handler)SVC_ke_free)(val);
                        break;
                    }
                ...
                // allocate value data
                val = (struct diss_val_elmt *) ((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct diss_val_elmt) + param->length, KE_MEM_ATT_DB);
                val->value = param->value;
                val->length = param->length;
                memcpy(val->data, param->data, param->length);
                // insert value into the list
                ((co_list_push_back_handler)SVC_co_list_push_back)(&(diss_env->values), &(val->hdr));
            }
        }
        else
        {
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    ke_state_t state = ((ke_state_get_handler)SVC_ke_state_get)(dest_id);
    ...
    if(state == DISS_IDLE)
    {
        ...
        if (value < DIS_CHAR_MAX)
        {
            ...
            else
            {
                ...
                req_ind->value = value;
                ((ke_msg_send_handler)SVC_ke_msg_send)(req_ind);

                // Put Service in a busy state
                ((ke_state_set_handler)SVC_ke_state_set)(dest_id, DISS_BUSY);
            }
        }
        else
        {
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if(state == DISS_BUSY)
    {
        ...
        ((ke_state_set_handler)SVC_ke_state_set)(dest_id, DISS_IDLE);
    }
...
void diss_task_init(struct ke_task_desc *task_desc)
{
	...
	task_desc->default_handler = &diss_default_handler;

    //task_desc->msg_handler_tab = diss_msg_handler_tab;
    task_desc->msg_cnt         = ARRAY_LEN(diss_default_state);
