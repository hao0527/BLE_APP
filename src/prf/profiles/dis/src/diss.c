...
static uint8_t diss_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct diss_db_cfg* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        struct diss_env_tag* diss_env =
                (struct diss_env_tag* ) ((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct diss_env_tag), KE_MEM_ATT_DB);
        ...
        ((co_list_init_handler)SVC_co_list_init)(&(diss_env->values));

        // service is ready, go into an Idle state
        ((ke_state_set_handler)SVC_ke_state_set)(env->task, DISS_IDLE);
    }
...
static void diss_destroy(struct prf_task_env* env)
{
    ...
    {
        struct co_list_hdr* hdr = ((co_list_pop_front_handler)SVC_co_list_pop_front)(&(diss_env->values));
        ((ke_free_handler)SVC_ke_free)(hdr);
    }

    // free profile environment variables
    env->env = NULL;
    ((ke_free_handler)SVC_ke_free)(diss_env);
}
