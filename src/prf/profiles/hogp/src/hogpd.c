...
const struct attm_desc hids_att_db[HOGPD_ATT_MAX] =
{
    ...
    // Boot Mouse Input Report Characteristic Declaration		32
    [HOGPD_IDX_BOOT_MOUSE_IN_REPORT_CHAR]       = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // Boot Mouse Input Report Characteristic Value		33
    ...
    // Boot Mouse Input Report Characteristic - Client Characteristic Configuration Descriptor		34
    ...
    // Report Characteristic Declaration		37
    [HOGPD_IDX_REPORT_CHAR]                     = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // Report Characteristic Value		38
    [HOGPD_IDX_REPORT_VAL]                      = {ATT_CHAR_REPORT, PERM(RD, ENABLE), PERM(RI, ENABLE), HOGPD_REPORT_MAX_LEN},
    // Report Characteristic - Report Reference Descriptor		39
    [HOGPD_IDX_REPORT_REP_REF]                  = {ATT_DESC_REPORT_REF, PERM(RD, ENABLE), 0, sizeof(struct hids_report_ref)},
    // Report Characteristic - Client Characteristic Configuration Descriptor		40
    [HOGPD_IDX_REPORT_NTF_CFG]                  = {ATT_DESC_CLIENT_CHAR_CFG,  PERM(RD, ENABLE)|PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE), 0, 0},

	    // Report Characteristic Declaration		41
    [HOGPD_IDX_REPORT_CHAR + 4]                     = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // Report Characteristic Value		42
    [HOGPD_IDX_REPORT_VAL + 4]                      = {ATT_CHAR_REPORT, PERM(RD, ENABLE), PERM(RI, ENABLE), HOGPD_REPORT_MAX_LEN},
    // Report Characteristic - Report Reference Descriptor		43
    [HOGPD_IDX_REPORT_REP_REF + 4]                  = {ATT_DESC_REPORT_REF, PERM(RD, ENABLE), 0, sizeof(struct hids_report_ref)},
    // Report Characteristic - Client Characteristic Configuration Descriptor		44
    [HOGPD_IDX_REPORT_NTF_CFG + 4]                  = {ATT_DESC_CLIENT_CHAR_CFG,  PERM(RD, ENABLE)|PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE), 0, 0},
};
...
static uint8_t hogpd_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct hogpd_db_cfg* params)
{
    ...
    hogpd_env = (struct hogpd_env_tag* ) ((ke_malloc_handler)SVC_ke_malloc)(sizeof(struct hogpd_env_tag), KE_MEM_ATT_DB);
    ...
    for (svc_idx = 0; ((svc_idx < params->hids_nb) && (status == GAP_ERR_NO_ERROR)); svc_idx++)
    {
        ...
        hids_db[svc_idx] = (struct attm_desc *) ((ke_malloc_handler)SVC_ke_malloc)(HOGPD_ATT_MAX * sizeof(struct attm_desc), KE_MEM_NON_RETENTION);
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        ((ke_state_set_handler)SVC_ke_state_set)(env->task, HOGPD_IDLE);
		
    }
    else if(hogpd_env != NULL)
    {
        ((ke_free_handler)SVC_ke_free)(hogpd_env);
    }
    ...
    for (svc_idx = 0; svc_idx < HOGPD_NB_HIDS_INST_MAX; svc_idx++)
    {
        ...
        {
            ((ke_free_handler)SVC_ke_free)(hids_db[svc_idx]);
        }
    }
...
uint8_t hogpd_ntf_send(uint8_t conidx, const struct hogpd_report_info* report)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        ((ke_msg_send_handler)SVC_ke_msg_send)(report_ntf);
    }
...
uint8_t hogpd_ntf_cfg_ind_send(uint8_t conidx, uint8_t svc_idx, uint8_t att_idx, uint8_t report_idx, uint16_t ntf_cfg)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        ((ke_msg_send_handler)SVC_ke_msg_send)(ntf_cfg_ind);
    }
