                                    ...
                                    ke_task_id_t const src_id)
{
    ...
    if((param->conidx > BLE_CONNECTION_MAX) || (((gapc_get_conhdl_handler)SVC_gapc_get_conhdl)(param->conidx) == GAP_INVALID_CONHDL))
    {
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if((state & HOGPD_REQ_BUSY) == HOGPD_IDLE)
    {
        ...
        {
            ((ke_state_set_handler)SVC_ke_state_set)(dest_id, state | HOGPD_REQ_BUSY);
        }
    }
                                    ...
                                    ke_task_id_t const src_id)
{
    ...
    if((state & HOGPD_OP_BUSY) == HOGPD_OP_BUSY)
    {
        ...
        if(hogpd_env->op.operation == HOGPD_OP_REPORT_READ)
        {
            ...
            ((ke_msg_send_handler)SVC_ke_msg_send)(rd_cfm);
        }
        // write operation
        else
        {
            ...
            ((ke_msg_send_handler)SVC_ke_msg_send)(wr_cfm);
        }
        ...
        ((ke_state_set_handler)SVC_ke_state_set)(dest_id, state & ~HOGPD_OP_BUSY);
    }
        ...
        if(status == GAP_ERR_NO_ERROR)
        {
            ...
            switch(att_idx)
            {
                ...
                case HOGPD_IDX_HID_CTNL_PT_VAL:
                {
                    ...
                    ((ke_msg_send_handler)SVC_ke_msg_send)(cp_ind);
                }break;
                ...
                case HOGPD_IDX_PROTO_MODE_VAL:
                {
                    ...
                    ((ke_msg_send_handler)SVC_ke_msg_send)(prot_ind);
                ...
                case HOGPD_IDX_REPORT_VAL:
                {
                    ...
                    ((ke_msg_send_handler)SVC_ke_msg_send)(report_ind);
                                 ...
                                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    if(param->operation == GATTC_NOTIFY)
    {
        ...
        ((ke_state_set_handler)SVC_ke_state_set)(dest_id, ((ke_state_get_handler)SVC_ke_state_get)(dest_id) & ~HOGPD_REQ_BUSY);
...
void hogpd_task_init(struct ke_task_desc *task_desc)
{
	...
	task_desc->default_handler = &hogpd_default_handler;

//    task_desc->msg_handler_tab = hogpd_msg_handler_tab;
    task_desc->msg_cnt         = ARRAY_LEN(hogpd_default_state);
